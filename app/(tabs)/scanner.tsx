...
export default function ScannerScreen() {
  ...
  const styles = useMemo(() => createScannerStyles(C), [C]);
    ...
    return (
      <View style={styles.container}>
        <View style={{ alignItems: 'center', marginBottom: 24 }}>
          <Ionicons name="camera" size={64} color={C.primary} />
        </View>
        <Text style={{ color: C.primary, textAlign: 'center', marginBottom: 24, fontSize: 18, fontWeight: '800' }}>
          {t('scanner.title')}
        </Text>
        <Text style={{ color: C.textLight, textAlign: 'center', marginBottom: 32, fontSize: 14, lineHeight: 22 }}>
        ...
        >
          <Text style={{ color: C.cardWhite, fontWeight: '700' }}>{t('scanner.enableBtn')}</Text>
        </TouchableOpacity>

        <TouchableOpacity onPress={() => router.push('/')} style={{ paddingVertical: 18, marginTop: 12 }}>
          <Text style={{ color: C.primary, fontWeight: '700' }}>{t('scanner.backBtn')}</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#000' }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 24, paddingTop: 60, zIndex: 10 }}>
        <Text style={{ color: C.cardWhite, fontSize: 18, fontWeight: '800' }}>{t('scanner.hint')}</Text>
        <TouchableOpacity onPress={() => router.push('/')}>
          <Ionicons name="close-circle" size={32} color={C.cardWhite} />
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.overlayHint} pointerEvents="none">
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 40 }}>
          ...
          <Text style={{ color: C.cardWhite, fontWeight: '800', marginLeft: 8, fontSize: 15 }}>{t('scanner.searching')}</Text>
        </View>
      </View>
    </View>
  );
...
function createScannerStyles(C: ColorPalette) {
  return StyleSheet.create({
    container: { flex: 1, backgroundColor: C.overlayDark, justifyContent: 'center', alignItems: 'center', padding: 32 },
