...
export default function OcrProcessScreen() {
  ...
  const styles = useMemo(() => createOcrProcessStyles(C), [C]);
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.back()} style={styles.backBtn}>
          <Ionicons name="arrow-back" size={24} color={C.textLight} />
        </TouchableOpacity>
      ...
      {step === 'launching' && (
        <View style={styles.centerBox}>
          <ActivityIndicator size="large" color={C.slate} />
      ...
      {step === 'ocr_done' && (
        <View style={styles.centerBox}>
          <View style={styles.successIconBox}>
            <Ionicons name="checkmark-circle" size={56} color={C.connectivity.online} />
          </View>
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={styles.assetCard} onPress={() => selectAsset(item.id)}>
                <View style={styles.assetIcon}>
                  <Ionicons name="cube-outline" size={20} color={C.slate} />
                </View>
                <View style={{ flex: 1 }}>
...
function createOcrProcessStyles(C: ColorPalette) {
  return StyleSheet.create({
    ...
    headerTitle: { fontSize: 16, fontWeight: '800', color: C.slate },
    centerBox: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 24 },
    infoText: { marginTop: 16, fontSize: 14, color: C.textSecondary, fontWeight: '700' },
    imageScanWrap: {
      width: width * 0.7,
      height: width * 1.1,
      borderRadius: 24,
    ...
    successTitle: { fontSize: 24, fontWeight: '900', color: C.slate, marginBottom: 24 },
    ocrCard: {
      width: '100%',
    ...
    ocrLabel: { fontSize: 11, fontWeight: '800', color: C.textSecondary, textTransform: 'uppercase', marginBottom: 4 },
    ocrValue: { fontSize: 20, fontWeight: '900', color: C.slate },
    actionBtn: {
      width: '100%',
    ...
    pickTitle: { fontSize: 22, fontWeight: '900', color: C.slate, marginTop: 24, marginBottom: 6 },
    pickSub: { fontSize: 14, color: C.textSecondary, marginBottom: 24 },
    assetCard: {
      flexDirection: 'row',
      alignItems: 'center',
      padding: 16,
    ...
    assetName: { fontSize: 15, fontWeight: '800', color: C.slate },
    assetType: { fontSize: 12, color: C.textSecondary, marginTop: 2, fontWeight: '600' },
  });
}
