...
const MediaCard = React.memo(({ item, t, themeColors, cardStyles, onDelete, onPress }: {
  ...
  if (item.stampedGeo && coords) {
    stampLines.push(formatCoords(coords[0], coords[1], 4));
  }
...
export default function MediaScreen() {
  ...
  const viewerCoords = viewItem ? getValidCoords(viewItem.latitude, viewItem.longitude) : null;
            ...
            <View style={styles.viewerContent}>
              ...
              {(viewItem.stampedGeo || viewItem.stampedDatetime) && (
                <View style={styles.viewerStamp} pointerEvents="none">
                  ...
                  {viewItem.stampedGeo && viewerCoords && (
                    <Text style={[styles.viewerStampText, { opacity: 0.8 }]}>
                      {formatCoords(viewerCoords[0], viewerCoords[1], 6)}
                    </Text>
                  )}
                </View>
              )}
