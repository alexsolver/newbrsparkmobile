...
import { apiFetch, userHasCapability, isB2CConsumerUser } from '../../../src/services/auth';
...
/**
 * Aba «Iniciadas»: toda a FT em `PAUSED` fica no topo, sem quebrar a ordem relativa
 * (entre pausadas nem o resto — o mesmo fim de `sortTasksForOsrmRoute` para cada grupo).
 */
function providerIniciadasListPauseAtTop(
  alreadySorted: any[],
  ...
  const paused: any[] = [];
  const other: any[] = [];
  for (const t of alreadySorted) {
    if (String(effectiveProviderTaskStatus(t, completedIds, inprogressIds, acceptedIds) || '').toUpperCase() === 'PAUSED') {
      paused.push(t);
    } else {
      other.push(t);
    }
...
function providerTaskListAccentColor(
  ...
  /** Pendentes — azul (alinhado ao hub do checklist e barra do card studio). */
  return MEDIA_TAG_COLORS.BEFORE;
}
...
function providerCardPremiumTone(
  ...
  const pendingCardBg = highContrast ? '#DBEAFE' : '#EFF6FF';
  ...
  return applyOverdue({
    ...base,
    stripe: MEDIA_TAG_COLORS.BEFORE,
    cardBg: pendingCardBg,
    cardBorder: highContrast ? `${MEDIA_TAG_COLORS.BEFORE}7A` : `${MEDIA_TAG_COLORS.BEFORE}45`,
    idBg: highContrast ? `${MEDIA_TAG_COLORS.BEFORE}22` : `${MEDIA_TAG_COLORS.BEFORE}18`,
    idBorder: highContrast ? `${MEDIA_TAG_COLORS.BEFORE}6E` : `${MEDIA_TAG_COLORS.BEFORE}40`,
    idInk: C.slate,
    statusBg: highContrast ? `${MEDIA_TAG_COLORS.BEFORE}1E` : `${MEDIA_TAG_COLORS.BEFORE}14`,
    statusBorder: highContrast ? `${MEDIA_TAG_COLORS.BEFORE}55` : `${MEDIA_TAG_COLORS.BEFORE}38`,
    statusInk: MEDIA_TAG_COLORS.BEFORE,
  });
}

/** Cor da fina barra no topo (design «studio»): azul=pendente, âmbar=em andamento, verde=concluída, vermelho=pausa. */
function providerCardStudioTopBarColor(listEff: string, C: ColorPalette): string {
  if (listEff === 'PAUSED') return C.destructive;
  if (listEff === 'COMPLETED') return C.status.success.fg;
  if (listEff === 'IN_PROGRESS') return MEDIA_TAG_COLORS.DURING;
  return MEDIA_TAG_COLORS.BEFORE;
}

type ProviderCardPremiumTone = ReturnType<typeof providerCardPremiumTone>;

/**
 * Chip de estado: cor da faixa (`providerCardStudioTopBarColor`) + borda alinhada ao contorno do card
 * (`cardTone.cardBorder`). Só a semântica do estado (pend. / andam. / concl. / pausa) — nunca forçar vermelho
 * por vencimento, senão a maioria dos cartões fica tudo "alerta" (muitas OS têm venc. no passado no cartão).
 * O vencimento continua visível na cor da borda do card, na linha de venc. e noutros avisos.
 */
type ProviderStatusChip = {
  backgroundColor: string;
  borderColor: string;
  textColor: string;
  /** Sombra mínima no amarelo — letras brancas; sem halo o contraste caía abaixo de WCAG. */
  textStyle?: { textShadowColor: string; textShadowOffset: { width: number; height: number }; textShadowRadius: number };
};

function providerCardStatusChipSolid(
  listEff: string,
  cardTone: ProviderCardPremiumTone,
  C: ColorPalette,
  /** Texto concluído claro/escuro: verde de faixa muda de luminância em dark. */
  themeDark: boolean,
): ProviderStatusChip {
  const fill = providerCardStudioTopBarColor(listEff, C);
  if (listEff === 'IN_PROGRESS') {
    return {
  ...
  const textColor: string = listEff === 'COMPLETED' ? (themeDark ? '#0A1914' : '#FFFFFF') : '#FFFFFF';
  return {
    backgroundColor: fill,
    borderColor: cardTone.cardBorder,
    textColor,
...
export default function DashboardScreen() {
  ...
  const loadData = (triggerSync = false) => {
    ...
    // Ativos: apenas para usuários autenticados
    if (user) {
  ...
  const renderMapContent = () => {
    ...
    const visibleAssets = pool.filter(a =>
      activeFilter === 'ALL' || activeFilter === 'GLOBAL' || assetMatchesRootFilter(a.type, activeFilter)
    );
    ...
    const TYPE_COLORS: Record<string, string> = {
      ...
      COLLECTIONS: MEDIA_TAG_COLORS.DURING,
      ...
      <View style={{ flex: 1, minHeight: 500 }}>
        {mappable.length === 0 ? (
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingTop: 80 }}>
            ...
            <Text style={{ fontSize: 15, fontWeight: '800', color: C.textSecondary, marginTop: 16 }}>Nenhum ativo tem localização GPS</Text>
            <Text style={{ fontSize: 12, color: C.textLight, marginTop: 6, textAlign: 'center', paddingHorizontal: 32 }}>
              Adicione coordenadas ao cadastrar um ativo para visualizá-lo no mapa.
            </Text>
          </View>
        ) : (
          <View style={{ position: 'relative' }}>
            ...
            >
              {mappable.map(({ asset, coords }) => {
                ...
                const typeIcon = (getAssetRootIconColor(asset.type).icon || 'star') as any;
                const labels: Record<string, string> = {
                  REAL_ESTATE: t('asset.type.REAL_ESTATE'),
                  MOBILITY: t('asset.type.MOBILITY'),
                  TERRESTRIAL: t('asset.type.TERRESTRIAL'),
                  MACHINERY: t('asset.type.MACHINERY'),
                  AQUATIC: t('asset.type.AQUATIC'),
                  IT: t('asset.type.IT'),
                  COLLECTIONS: t('asset.type.COLLECTIONS'),
                  SPECIAL: t('asset.type.SPECIAL'),
                        ...
                        <View style={{
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                            ...
                            <Text style={{ fontSize: 10, fontWeight: '800', color: C.status.warning.fg, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                              {childCount} ativo{childCount > 1 ? 's' : ''} vinculado{childCount > 1 ? 's' : ''}
                            </Text>
                          </View>
  ...
  const renderAssetContent = () => {
    const pool = activeFilter === 'GLOBAL' ? allAssets : assets;
    let visibleAssets = pool.filter(a => activeFilter === 'ALL' || activeFilter === 'GLOBAL' || assetMatchesRootFilter(a.type, activeFilter));
    ...
    return (
      <View>
          ...
          <View style={{ padding: 16, backgroundColor: C.status.info.bg, borderWidth: 1, borderColor: C.status.info.border, borderRadius: 16, marginHorizontal: 16, marginBottom: 20 }}>
            ...
            {pendingShares.map(ps => (
              <View key={ps.id} style={{ backgroundColor: C.cardWhite, padding: 12, borderRadius: 12, borderWidth: 1, borderColor: C.status.info.border, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                 <View style={{ flex: 1 }}>
                   <Text style={{ fontSize: 13, fontWeight: '800', color: C.slate, marginBottom: 2 }}>{ps.asset?.title || 'Ativo compartilhado'}</Text>
        ...
        {renderAssetList(myAssets, sharedAssets.length > 0 || pendingShares.length > 0 ? 'Meus ativos' : undefined, 'MY')}
  ...
  const technicianProviderPagerLocked = providerModeOnly || (canUseProviderModeInUi && mode === 'PROVIDER');
        ...
        >
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScroll} style={{marginBottom: 16}} keyboardShouldPersistTaps="handled">
             {[
                ...
                { id: 'MOBILITY',    label: t('home.assetTypes.mobility') },
                { id: 'MACHINERY',   label: t('home.assetTypes.machinery') },
                { id: 'AQUATIC',     label: t('home.assetTypes.aquatic') },
                { id: 'IT',          label: t('home.assetTypes.it') },
                { id: 'COLLECTIONS', label: t('home.assetTypes.collections') },
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              >
                ...
                const isStudioLayout = isPremiumCard;
                const statusChipSolid = providerCardStatusChipSolid(listEff, cardTone, C, themeDark);
                const studioTopBar = providerCardStudioTopBarColor(listEff, C);
                const studioReceiptValue = formatProviderTaskLocaleDateTime(
                  providerTaskDeviceReceivedAtIso(order),
                  i18n.language,
                );
                const studioDueValue = cardVencIso
                  ...
                  >
                    {isStudioLayout ? (
                      <ProviderOsListCardStudio
                        ...
                        onMapPress={() => runProviderOsMapMiniPress(order, setProviderOsMiniMapTask)}
                        onRevisionPress={() => {
                          setSelectedTask(order);
                          setTaskModalVisible(true);
                        }}
                        hasRevision={providerTaskShowsRevisionBadge(order)}
                        cardStatusChipText={cardStatusChipText}
                        statusChip={statusChipSolid}
                        formBadgeText={formBadgeText}
                        hasPendingVoiceNote={hasPendingVoiceNote}
                        showMetricsStrip={providerCardShowsFieldMetrics(listEff) && listEff !== 'PAUSED'}
                        hasDur={hasDurFooter}
                        durText={hasDurFooter && durMinFooter != null ? `${durMinFooter} min` : '—'}
                        showDist={!!showDistFooter}
                        distLoading={
                          Boolean(
                            showDistFooter &&
                        ...
                        distValueText={
                          showDistFooter && providerMyLocation && providerCardMapDest
                            ? formatHaversineForUi(haversineMeters(providerMyLocation, providerCardMapDest))
                            : ''
                        }
                        showEta={!!showProviderEtaChip}
                        etaText={showProviderEtaChip && etaChipMinutes != null ? `${etaChipMinutes} min` : '—'}
                        studioReceiptValue={studioReceiptValue}
                        studioDueValue={studioDueValue}
                        cardVencOverdue={cardVencOverdue}
                        cardVencIso={cardVencIso}
                        mapAccessibilityLabel={`${osMapZoneVisual.label}. Ver local no mapa`}
                              ...
                              <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
                                ...
                                {isPremiumCard ? (
                                  <View
                                    style={{
                                      alignSelf: 'flex-start',
                                      backgroundColor: statusChipSolid.backgroundColor,
                                      borderColor: statusChipSolid.borderColor,
                                      borderWidth: 1,
                                      borderRadius: 999,
                                      ...
                                      ...Platform.select({
                                        ios: {
                                          shadowColor: statusChipSolid.backgroundColor,
                                          shadowOffset: { width: 0, height: 2 },
                                          shadowOpacity: 0.4,
                                          shadowRadius: 5,
                                        },
                                      ...
                                      style={[
                                        {
                                          fontSize: 10,
                                          fontWeight: '900',
                                          color: statusChipSolid.textColor,
                                          letterSpacing: 0.25,
                                        },
                                        statusChipSolid.textStyle,
                                      ]}
                                    >
                                      {cardStatusChipText}
                                    </Text>
           ...
           <View style={{ height: 110, backgroundColor: C.overlayDark, paddingTop: 50, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', zIndex: 10 }}>
            ...
            {showRouteMap && (
             <MapView
               ...
               style={{ flex: 1 }}
               initialRegion={{
                 latitude: routeMapCenterObj?.lat ?? parseCoordLatLng(routeMapTasksOrdered[0])?.lat ?? -23.5505,
                 longitude: routeMapCenterObj?.lng ?? parseCoordLatLng(routeMapTasksOrdered[0])?.lng ?? -46.6333,
                 latitudeDelta: 0.12,
                 longitudeDelta: 0.12
               }}
             showsUserLocation
