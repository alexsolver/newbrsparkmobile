#!/usr/bin/env node
/**
 * Move todos os User para o tenant com slug "brspark".
 * Resolve @@unique([email, tenantId]) com e-mail alternativo em colisão.
 *
 * Uso: node scripts/moveAllUsersToBrsparkTenant.js
 *      node scripts/moveAllUsersToBrsparkTenant.js --dry-run
 */
'use strict';
...
function nextUniqueEmail(email, usedLower) {
  ...
  if (at <= 0) {
    const alt = `migrado_${Date.now()}@brspark.com`;
...
async function main() {
  ...
  if (!target) {
    console.error('Tenant com slug "brspark" não encontrado.');
    process.exit(1);
  }
  ...
  if (toMove.length === 0) {
    console.log('Nada a fazer — todos já estão no tenant brspark.');
    return;
  }
