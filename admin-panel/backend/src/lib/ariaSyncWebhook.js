 ...
 */
function deliverAriaLaravelEvent(opts) {
  const { type, payload, id, idempotencyKey } = opts;
  const base = String(
    process.env.ARIA_LARAVEL_BASE_URL || process.env.LARAVEL_ARIA_SYNC_BASE_URL || ''
  )
    .trim()
    .replace(/\/+$/, '');
  const path = String(process.env.ARIA_LARAVEL_WEBHOOK_PATH || '/api/internal/aria/events');
  const fullUrl = base ? `${base}${path.startsWith('/') ? path : `/${path}`}` : '';
  const secret = String(process.env.ARIA_WEBHOOK_SECRET || '').trim();
  const token = String(process.env.CMS_INTERNAL_API_TOKEN || process.env.ARIA_LARAVEL_BEARER || '').trim();
  ...
  return new Promise((resolve) => {
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
            ...
            console.warn('[ariaSyncWebhook] falha', res.statusCode, txt?.slice(0, 240));
    ...
    req.on('error', (e) => {
      console.warn('[ariaSyncWebhook] rede', e?.message);
