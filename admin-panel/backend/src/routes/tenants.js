...
function readTenantFeaturesJson(features) {
  if (features && typeof features === 'object' && !Array.isArray(features)) return features;
  return {};
}

/**
 * GET /api/tenants/:id/provider-affiliation-onboarding-config
 * Campos extra / Didit / skipFace para onboarding web (tenant admin ou plataforma).
 */
router.get('/:id/provider-affiliation-onboarding-config', async (req, res) => {
  try {
    const tenantId = String(req.params.id || '').trim();
    if (!requireTenantRouteAccess(req, tenantId, res)) return;
    const enabled = await isProviderFirstNetworkEnabled(tenantId);
    if (!enabled) {
      return res.status(403).json({
        error: 'Fluxo provider-first desactivado para este tenant.',
        code: 'PROVIDER_FIRST_DISABLED',
      });
    }
    const tenant = await prisma.tenant.findUnique({
    ...
    if (!tenant) return res.status(404).json({ error: 'Tenant não encontrado.' });
    const f = readTenantFeaturesJson(tenant.features);
    return res.json({
      providerAffiliationOnboarding: f.providerAffiliationOnboarding || {},
      providerOnboardingDidit: f.providerOnboardingDidit || {},
    });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
...
/**
 * PATCH /api/tenants/:id/provider-affiliation-onboarding-config
 * Body: { providerAffiliationOnboarding?: object, providerOnboardingDidit?: object } — merge superficial.
 */
router.patch('/:id/provider-affiliation-onboarding-config', async (req, res) => {
  try {
    const tenantId = String(req.params.id || '').trim();
    if (!requireTenantRouteAccess(req, tenantId, res)) return;
    const enabled = await isProviderFirstNetworkEnabled(tenantId);
    ...
    const patchOb = req.body?.providerAffiliationOnboarding;
    const patchDi = req.body?.providerOnboardingDidit;
    if (
      (patchOb != null && typeof patchOb !== 'object') ||
      (patchDi != null && typeof patchDi !== 'object') ||
      (patchOb == null && patchDi == null)
    ) {
      return res.status(400).json({
        error: 'Envie providerAffiliationOnboarding e/ou providerOnboardingDidit como objectos.',
      });
    }
    const tenant = await prisma.tenant.findUnique({
      where: { id: tenantId },
    ...
    if (!tenant) return res.status(404).json({ error: 'Tenant não encontrado.' });
    const cur = readTenantFeaturesJson(tenant.features);
    const next = { ...cur };
    if (patchOb && typeof patchOb === 'object' && !Array.isArray(patchOb)) {
      next.providerAffiliationOnboarding = {
        ...(typeof cur.providerAffiliationOnboarding === 'object' && cur.providerAffiliationOnboarding
          ? cur.providerAffiliationOnboarding
          : {}),
        ...patchOb,
      };
    }
    if (patchDi && typeof patchDi === 'object' && !Array.isArray(patchDi)) {
      next.providerOnboardingDidit = {
        ...(typeof cur.providerOnboardingDidit === 'object' && cur.providerOnboardingDidit
          ? cur.providerOnboardingDidit
          : {}),
        ...patchDi,
      };
    }
    ...
    await auditFromReq(req, 'TENANT_PROVIDER_WEB_ONBOARDING_CONFIG', tenant.name || tenantId, tenantId);
