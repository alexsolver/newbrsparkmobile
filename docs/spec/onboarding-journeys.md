...
2. Com a tenant COMPANY partilhada resolvida por `APP_REGISTRATION_SHARED_TENANT_*` (omissão de slug: `master` no seed), o utilizador é criado na **tenant partilhada** «Aria App» com papel **USER** — não cria empresa própria.
...
**Seed:** tenant `slug: master` em `src/seed.js` (renomeia legado `aria-app` se existir).

## 2. Prestador (técnico)

1. Mesmo registo que o cliente (tenant padrão Aria App).
...
1. No app, **Perfil** → secção **Cadastrar minha empresa** → abre o portal web (por defeito `https://www.aria.com/empresa`).
2. URL configurável: `EXPO_PUBLIC_ARIA_COMPANY_SIGNUP_URL` no `.env` do projeto Expo.
