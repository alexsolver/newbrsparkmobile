...
const TAG_COLORS: Record<string, string> = {
  BEFORE: '#3B82F6', DURING: '#F59E0B', AFTER: '#10B981',
  DAMAGE: '#EF4444', WARRANTY: '#8B5CF6', OTHER: '#6B7280',
};
      ...
      <View style={S.card}>
        <View style={S.thumbWrap}>
          {item.type === 'photo'
            ...
            : (
              <View style={[S.thumb, S.videoBox]}>
                <Ionicons name="videocam" size={28} color={C.textSecondary} />
              </View>
            )
          }
          {stampLines.length > 0 && item.type === 'photo' && (
            <View style={S.stampOverlay} pointerEvents="none">
              {stampLines.map((l, i) => <Text key={i} style={S.stampTxt} numberOfLines={1}>{l}</Text>)}
            </View>
          )}
          {item.type === 'video' && (
          ...
          {!!item.tag && (() => {
            ...
            const color = TAG_COLORS[clean] ?? TAG_COLORS.OTHER;
            return clean ? (
              <View style={[S.tagBadge, { backgroundColor: color }]}>
                <Text style={S.tagBadgeTxt}>{t(`media.tags.${clean}`)}</Text>
              </View>
            ) : null;
          })()}
          <TouchableOpacity
        ...
        <View style={S.cardMeta}>
          ...
          {item.stampedGeo && (
            <View style={S.metaRow}>
              <Ionicons name="location" size={9} color={C.textSecondary} />
          ...
          {item.stampedDatetime && (
            <View style={S.metaRow}>
              <Ionicons name="time" size={9} color={C.textSecondary} />
              <Text style={S.metaTxt}>{new Date(item.createdAt).toLocaleDateString('pt-BR')}</Text>
            </View>
          )}
        </View>
      </View>
  ...
  return (
    <View style={S.toggleRow}>
      ...
      <Switch
        ...
        thumbColor={value ? C.accent : '#ccc'}
        trackColor={{ false: '#e5e7eb', true: C.accent + '66' }}
      />
    </View>
  );
}
              ...
              <ScrollView contentContainerStyle={{ padding: 20 }} keyboardShouldPersistTaps="handled">
                ...
                <View style={S.tagRow}>
                  {TAG_KEYS.map(k => {
                    const sel = editTag === k;
                    return (
                      <TouchableOpacity key={k} style={[S.tagChip, { borderColor: TAG_COLORS[k] }, sel && { backgroundColor: TAG_COLORS[k] }]} onPress={() => setEditTag(sel ? '' : k)} activeOpacity={0.7}>
                        <Text style={[S.tagChipTxt, { color: sel ? '#fff' : TAG_COLORS[k] }]}>{t(`media.tags.${k}`)}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
            ...
            {!pending ? (
              ...
              <>
                ...
                <View style={S.toggleCard}>
                  <ToggleRow label={t('media.geoStamp')} sub={detecting ? t('media.detectingGeo') : t('media.geoStampSub')} value={stampGeo} onChange={handleGeoToggle} />
                  {stampGeo && geoData && (
                    <Text style={S.geoHint}>📍 {geoData.address} · {geoData.latitude.toFixed(4)}, {geoData.longitude.toFixed(4)}</Text>
                  )}
                  <View style={S.divider} />
                  <ToggleRow label={t('media.datetimeStamp')} sub={t('media.datetimeStampSub')} value={stampDt} onChange={setStampDt} />
                </View>
                ...
                <View style={S.tagRow}>
                  {TAG_KEYS.map(k => {
                    const sel = tag === k;
                    return (
                      <TouchableOpacity key={k} style={[S.tagChip, { borderColor: TAG_COLORS[k] }, sel && { backgroundColor: TAG_COLORS[k] }]} onPress={() => setTag(sel ? '' : k)} activeOpacity={0.7}>
                        <Text style={[S.tagChipTxt, { color: sel ? '#fff' : TAG_COLORS[k] }]}>{t(`media.tags.${k}`)}</Text>
                      </TouchableOpacity>
                    );
                  })}
