      ...
      <View style={S.searchArea}>
         <View style={S.searchHeaderRow}>
            ...
            <TouchableOpacity style={[S.stdAddBtn, {backgroundColor: '#F1F5F9'}]} onPress={() => setCreateFolderModal(true)}>
               <Ionicons name="folder-open" size={20} color={themeColors.slate} />
            </TouchableOpacity>
         </View>
         ...
         {folderPath.length > 0 && (
           <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
             ...
             <Ionicons name="chevron-forward" size={14} color={themeColors.border} style={{ marginHorizontal: 4 }} />
             {folderPath.map((f, i) => (
               <React.Fragment key={f.id}>
                 <TouchableOpacity onPress={() => setCurrentFolderId(f.id)} style={{ padding: 4 }}>
                   <Text style={{ fontSize: 13, fontWeight: i === folderPath.length - 1 ? '800' : '600', color: i === folderPath.length - 1 ? themeColors.slate : themeColors.textSecondary }}>{f.title}</Text>
                 </TouchableOpacity>
                 {i < folderPath.length - 1 && <Ionicons name="chevron-forward" size={14} color={themeColors.border} style={{ marginHorizontal: 4 }} />}
               </React.Fragment>
             ))}
           </View>
         )}
      ...
      <View style={S.categorySection}>
        {filteredDocs.length === 0 && (
          <View style={{ alignItems: 'center', padding: 40 }}>
            ...
            <Text style={{ marginTop: 16, fontSize: 14, fontWeight: '700', color: themeColors.textSecondary }}>Pasta vazia</Text>
          </View>
        )}
        ...
        {filteredDocs.filter(d => d.type === 'folder').map(doc => (
          <TouchableOpacity key={doc.id} style={S.docCard} onPress={() => setCurrentFolderId(doc.id)}>
            <View style={S.docIconBox}>
              <Ionicons name="folder" size={22} color={themeColors.primary} />
            </View>
            <View style={{flex: 1}}>
        ...
        {filteredDocs.filter(d => d.type !== 'folder').map(doc => (
          <TouchableOpacity key={doc.id} style={S.docCard} onPress={() => openDocument(doc)}>
            <View style={S.docIconBox}>
              <Ionicons name={doc.uri?.toLowerCase().endsWith('.pdf') ? "document-text" : doc.type === 'image' ? "image" : "document"} size={22} color={themeColors.accent} />
            ...
            <View style={{flexDirection: 'row'}}>
              <TouchableOpacity onPress={() => Sharing.shareAsync(doc.uri)} style={S.editBtn}>
                <Ionicons name="share-outline" size={18} color={themeColors.accent} />
              </TouchableOpacity>
      ...
      <Modal visible={createFolderModal} transparent animationType="fade">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 }}>
          <View style={{ backgroundColor: '#fff', borderRadius: 20, padding: 20 }}>
             <Text style={{ fontSize: 18, fontWeight: '800', color: themeColors.slate, marginBottom: 16 }}>Nova Pasta</Text>
             <TextInput 
               style={S.input}
             ...
             <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 12, marginTop: 20 }}>
               <TouchableOpacity onPress={() => setCreateFolderModal(false)} style={{ padding: 12 }}>
                 <Text style={{ color: themeColors.textSecondary, fontWeight: '700' }}>Cancelar</Text>
               </TouchableOpacity>
               <TouchableOpacity onPress={saveFolder} style={{ backgroundColor: themeColors.primary, paddingHorizontal: 20, paddingVertical: 12, borderRadius: 10 }}>
          ...
          <View style={S.modalContent}>
            <View style={S.modalHeader}>
              ...
              <TouchableOpacity onPress={saveDoc} disabled={loading}>
                {loading ? <ActivityIndicator size="small" color={themeColors.accent} /> : <Text style={S.saveText}>{editingDoc ? t('docs.update') : t('common.save')}</Text>}
              </TouchableOpacity>
            </View>
              ...
              <View style={S.formGroup}>
                ...
                <View style={S.sourceGrid}>
                  <TouchableOpacity style={[S.sourceBtn, docType === 'file' && S.sourceBtnActive]} onPress={pickDocument}>
                    <Ionicons name="document-attach" size={24} color={docType === 'file' ? '#fff' : themeColors.accent} />
                  ...
                  <TouchableOpacity style={[S.sourceBtn, docType === 'image' && S.sourceBtnActive]} onPress={() => pickImage(false)}>
                    <Ionicons name="images" size={24} color={docType === 'image' ? '#fff' : themeColors.accent} />
                  ...
                  <TouchableOpacity style={S.sourceBtn} onPress={() => pickImage(true)}>
                    <Ionicons name="camera" size={24} color={themeColors.accent} />
                    <Text style={S.sourceText}>{t('costs.camera')}</Text>
                  </TouchableOpacity>
                </View>
                {uri ? (
                  <View style={S.fileFeedback}>
                    <Ionicons name="checkmark-circle" size={16} color={themeColors.success.text} />
...
const S = StyleSheet.create({
  container: { flex: 1, backgroundColor: themeColors.background, paddingTop: 12 },
  searchArea: { paddingHorizontal: 16, marginBottom: 15 },
  searchHeaderRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  searchBox: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 12, borderRadius: 12, borderWidth: 1, borderColor: themeColors.border },
  stdAddBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: themeColors.primary, justifyContent: 'center', alignItems: 'center', shadowColor: themeColors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 },
  fab: { position: 'absolute', bottom: 30, right: 20, zIndex: 10, width: 60, height: 60, borderRadius: 30, backgroundColor: themeColors.primary, justifyContent: 'center', alignItems: 'center', shadowColor: themeColors.primary, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 8, elevation: 6 },
  searchInput: { flex: 1, marginLeft: 10, fontSize: 11, fontWeight: '700' },
  filterScroll: { paddingVertical: 10, gap: 8 },
  filterChip: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: '#fff', borderWidth: 1, borderColor: themeColors.border },
  filterChipActive: { backgroundColor: themeColors.accent, borderColor: themeColors.accent },
  filterChipText: { fontSize: 8, fontWeight: '900', color: themeColors.textSecondary, letterSpacing: 0.8, textTransform: 'uppercase' },
  filterChipActiveText: { color: '#fff' },

  categorySection: { paddingHorizontal: 16, marginBottom: 20 },
  categoryTitle: { fontSize: 7, fontWeight: '900', color: themeColors.textSecondary, marginBottom: 10, letterSpacing: 1.5, textTransform: 'uppercase' },
  docCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 12, borderRadius: 16, marginBottom: 8, borderWidth: 1, borderColor: themeColors.border },
  docIconBox: { width: 44, height: 44, borderRadius: 10, backgroundColor: themeColors.background, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  versionBadge: { position: 'absolute', top: -5, right: -5, backgroundColor: themeColors.accent, paddingHorizontal: 4, borderRadius: 5 },
  vText: { color: '#fff', fontSize: 8, fontWeight: '900' },
  docTitle: { fontSize: 11, fontWeight: '900', color: themeColors.primary, letterSpacing: -0.2, textTransform: 'uppercase' },
  docDate: { fontSize: 7, color: themeColors.textSecondary, marginTop: 2, fontWeight: '700', textTransform: 'uppercase' },
  editBtn: { padding: 8 },

  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' },
  modalContent: { backgroundColor: themeColors.background, borderTopLeftRadius: 30, borderTopRightRadius: 30, height: '92%' },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, backgroundColor: '#fff', borderTopLeftRadius: 30, borderTopRightRadius: 30, borderBottomWidth: 1, borderBottomColor: themeColors.border },
  headerTitle: { fontSize: 11, fontWeight: '900', color: themeColors.primary, letterSpacing: -0.2, textTransform: 'uppercase' },
  cancelText: { color: themeColors.textSecondary, fontWeight: '700', fontSize: 10, textTransform: 'uppercase' },
  saveText: { color: themeColors.accent, fontWeight: '900', fontSize: 11, letterSpacing: 0.5, textTransform: 'uppercase' },

  formScroll: { padding: 16 },
  formGroup: { backgroundColor: '#fff', borderRadius: 20, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: themeColors.border },
  groupLabel: { fontSize: 7, fontWeight: '900', color: themeColors.textSecondary, marginBottom: 12, letterSpacing: 1.2, textTransform: 'uppercase' },
  sourceGrid: { flexDirection: 'row', gap: 10 },
  sourceBtn: { flex: 1, paddingVertical: 15, borderRadius: 12, backgroundColor: themeColors.background, alignItems: 'center', gap: 6, borderWidth: 1, borderColor: themeColors.border },
  sourceBtnActive: { backgroundColor: themeColors.accent, borderColor: themeColors.accent },
  sourceText: { fontSize: 7, fontWeight: '900', color: themeColors.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5 },
  sourceTextActive: { color: '#fff' },
  fileFeedback: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 12, backgroundColor: themeColors.success.background, padding: 8, borderRadius: 8 },
  fileFeedbackText: { fontSize: 9, color: themeColors.success.text, fontWeight: '800' },

  input: { backgroundColor: themeColors.background, borderRadius: 12, padding: 14, fontSize: 11, fontWeight: '800', color: themeColors.primary, marginBottom: 10 },
  
  dateRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: themeColors.background, borderRadius: 12, padding: 10, gap: 10 },
  dateInput: { flex: 1, fontSize: 11, fontWeight: '900', color: themeColors.accent, textTransform: 'uppercase' },
  shiftGrid: { flexDirection: 'row', gap: 5 },
  shiftBtn: { backgroundColor: themeColors.accent, paddingHorizontal: 10, paddingVertical: 8, borderRadius: 8 },
  shiftText: { color: '#fff', fontSize: 7, fontWeight: '900' },
  subLabel: { fontSize: 7, fontWeight: '900', color: themeColors.textSecondary, marginTop: 15, textTransform: 'uppercase', letterSpacing: 0.8 },

  alertChip: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, backgroundColor: themeColors.background, marginRight: 8, borderWidth: 1, borderColor: themeColors.border },
  alertChipActive: { backgroundColor: themeColors.accent, borderColor: themeColors.accent },
  alertChipText: { fontSize: 7, fontWeight: '900', color: themeColors.textSecondary, textTransform: 'uppercase' },
  alertChipActiveText: { color: '#fff' },
  customBox: { flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 15, padding: 10, backgroundColor: themeColors.background, borderRadius: 12 },
  customLabel: { fontSize: 7, fontWeight: '900', color: themeColors.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5 },
  customInput: { borderBottomWidth: 2, borderBottomColor: themeColors.accent, width: 40, textAlign: 'center', fontSize: 12, fontWeight: '900', color: themeColors.accent },
  ...
  pdfM: { fontSize: 10, fontWeight: '800', color: themeColors.textSecondary, letterSpacing: 1 },
  pdfH: { fontSize: 20, fontWeight: '900', color: themeColors.primary, marginVertical: 10 },
  pdfB: { backgroundColor: themeColors.primary, paddingHorizontal: 30, paddingVertical: 15, borderRadius: 15, marginVertical: 15 },
  pdfBT: { color: '#fff', fontWeight: '800' },
  pdfC: { color: themeColors.textSecondary, fontWeight: '700', fontSize: 13 }
});
